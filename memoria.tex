...
\vspace{1cm}
\begin{flushright}
Fuenlabrada, a \qquad$\;\,$ de \qquad\qquad\qquad\qquad de 20CE
\end{flushright}
...
Y sobretodo gracias a mis abuelos, que al final siempre son los que más se preocupan por sus nietos y quienes más se alegran por su simple presencia. Otro motivo más para luchar al máximo y que puedan ver con sus propios ojos y decir con orgullo lo que su nieto ha hecho.
...
En este Trabajo Fin de Grado se desarrolla el programa SimpleRTP, cuya función es mediante un modelo API ofrecer una herramienta sencilla para enviar tráfico RTP con la configuración a gusto del usuario. Esto permite experimentar con el protocolo y probar las diferentes maneras de enviar paquetes RTP con configuraciones diversas e incluso provocar errores para comprobar los límites de este.

El proyecto nace de la asignatura de este grado PTAVI, con la propuesta de desarrollar un programa que cubra las necesidades de incluir de una forma práctica un programa que trate con mayor detalle el protocolo RTP respecto a lo que se hacía hasta el momento. RTP es un protocolo importante e interesante pero con la biblioteca que se usaba no se podía ver su funcionamiento, limitando las oportunidades en las prácticas de la asignatura. Se busca solventar esto permitiendo el control total de la cabecera RTP con funciones fáciles de usar, con diferentes grados de complejidad según se necesite.

En este proyecto hay varias tecnologías implicada, donde las más importantes para el estudio son RTP y MP3, pero se utilizan otras como el lenguaje de programación Python para desarrollar el programa y Wireshark como analizador de paquetes en la red para comprobar los resultados. La combinación de todas estas tecnologías es lo que da como resultado este proyecto y esta memoria donde más adelante se ve a fondo la arquitectura de este.
...
In this Thesis is developed the program SimpleRTP, whose function is throught an API model offer a simple tool for sending RTP traffic with the configuration to suit the user preferences. This permits to experiment with the protocol and test the diferent ways of sending RTP packets with various configurations and even cause errors to check the limits of the program.

The project was born form the the subject of this career PTAVI, with the proposal of developing a program that covers the needs to include in a practical way functions that deal with the protocol in grater detail regarding what was done so far. RTP is an insteresting and important protocol but with the actual library it was not possible to see how it works, limiting the scope of the subject practices. The program seeks to solve this by allowing full control of the RTP header with easy-to-use functions, with varying degrees of complexity as needed.

In this project there are various tecnologies involved, where the most important for the study are RTP and MP3, but others are used as the programing lenguaje Python for developing the program and Wireshark as a packet analyzer in the network for check the results. The combination of all these technologies is what results in this project and this memory where architecture is seen below in depth.
...
La realización de este proyecto contempla varios retos, entre ellos empezar un programa desde cero al no haber una base previa para el propósito del programa y encontrar la forma de que su utilizarlo sea a la vez sencillo para cualquier persona pero con el detalle suficiente para estudiar. A lo largo de la memoria se explica en mayor profundidad cada punto anteriormente citado y como se ha desarrollado.
...
\begin{itemize}
  \item El primer capítulo explica el origen de la iniactiva de este trabajo y el cómo y porqué se ha llevado a cabo.
...
\texttt{Bitstring} define un objeto que se puede construir con variables con una gran variedad de tipos, desde integer big y little endian hasta string, hexadecimal, byte y un proprio binario. 
...
\begin{verbatim}
    ...
    cabeceraRTP = simplertp.RtpHeader()
    csrc = [2000, 3000, 4000, 5000]
    cabeceraRTP.set_header(version=2, pad_flag=0, ext_flag=0, cc=4, marker=0, \emph{payload}_type=14, ssrc=1000)
    cabeceraRTP.setCSRC(csrc)
    audio = simplertp.RtpPayloadMp3('archivo.mp3')
    numeroPaquetesRTP = 0
    paquetesMP3porRTP = 2
