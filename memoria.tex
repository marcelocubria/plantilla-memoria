...
En la figura~\ref{fig:cronograma} se puede ver el cronograma del TFG.
...
Dentro de este estándar los ficheros de audio pueden tener diferente velocidad de bit rate (kbits/s) y de velocidad de muestreo (Hz). Ambos parámetros se definen en la cabecera del paquete con MP3 (RFC 5219~\cite{finlaysonrfc5219}).
...
Una vez tenemos en un objeto todo lo necesario para formar la cabecera de un paquete RTP, el siguiente paso es el \emph{payload} o la carga útil del paquete. Aquí es donde se encuentra el paquete con MP3. Un fichero MP3 es una sucesión continua de paquetes por lo que es necesario localizar la cabecera de cada paquete y analizarla para ver sus cualidades como el tiempo de muestreo o los bits por segundo.
...
El programa cliente puede tener cualquier propósito y diferentes implementaciones. Para el uso de SimpleRTP es necesario importar la biblioteca y también tener la biblioteca de bitstring. La biblioteca se puede encontrar en el repositorio de Github con dirección \url{https://github.com/marcelocubria/rtpmp3}.
